...
CYRAL_REPOSITORIES_SUPPORTED
    ...
    CYRAL_REPOSITORIES_SUPPORTED: a comma seperated list of wires you'd like enabled. if not set all wires are enabled.
...
disable_unsupported_services () {
  ...
  if [ -n "$CYRAL_SUPPORTED_WIRES" ]; then
    wires_to_disable=$(for wire in "${WIRES[@]}"; do if [[ ! "$CYRAL_SUPPORTED_WIRES" =~ $(echo "$wire"|cut -d- -f2) ]]; then echo -n "$wire "; fi; done)
    echo "Wires to be disabled: ${wires_to_disable[*]}"
  fi
...
restart_services () {
  ...
  readarray -t SERVICES < <(find /etc/cyral/ -type d -name "cyral-*" -printf "%f\n")
